...
mod tests {
    ...
    fn app_settings_serde_roundtrip() {
        ...
        let s2: AppSettings = serde_json::from_str(&text).unwrap();
        assert_eq!(s.history_days, s2.history_days);
        assert_eq!(s.auto_sync_on_send, s2.auto_sync_on_send);
